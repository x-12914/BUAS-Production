// Authentication Service for BUAS RBAC Frontend
...
class AuthService {
    constructor() {
        ...
        this.baseURL = `${this.apiBaseURL}/api/auth`;
        this.user = null;
        this.listeners = new Set();
    }
    
    // Authentication status listeners
    ...
    async getProfile() {
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                return {success: true, user: data.user, permissions: data.permissions};
            } else {
    ...
    async checkPasswordStrength(password, username = '') {
        try {
            const response = await fetch(`${this.baseURL}/check-password-strength`, {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
            ...
            if (response.ok) {
                ...
                return {success: false, error: 'Failed to check password strength'};
            }
        } catch (error) {
    ...
    async getPasswordRequirements() {
        try {
            const response = await fetch(`${this.baseURL}/password-requirements`);
            
            if (response.ok) {
                const data = await response.json();
                return {success: true, policy: data.policy, requirements: data.requirements};
            } else {
                return {success: false, error: 'Failed to load password requirements'};
            }
        } catch (error) {
    ...
    hasPermission(permission) {
        ...
        // Permission matrix from the implementation guide
    ...
    canAccessDevice(deviceId) {
        ...
        if (this.user.role === 'analyst') {
            // For analysts, check assigned devices
